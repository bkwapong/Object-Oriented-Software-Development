...
public class LeagueTableMain {
    ...
    public static void main(String[] args) {
		...
		//Use stream() to find the lowest points in the League.	
		
		Integer lowestPoints = teams.stream().
		...
		//Use stream() to find the highest points in the League.			
		Integer highestPoints = teams.stream().
			map((EnglishPremierLeagueTeam team) -> team.getTotalPoints()).
			reduce(0, (result, teamPoints) -> 
				{if (result == 0) 
					return teamPoints; 
		...
		Comparator<EnglishPremierLeagueTeam> byName = Comparator.comparing
				...
				//(EnglishPremierLeagueTeam::getTeamName).reversed();
		...
		//Print the lowest and highest points in the league
